...
<div class="container">
                ...
                <div class="col-lg-6 col-md-6">
                    ...
                    <div class="buttons group">
                        <div class="qnt-count" style="float:left;">
                            ...
                            <input id="quantity" class="form-control" type="text" value="<?php echo $product->availability;?>">

                        </div>
                        <!--Add To Cart Button-->
                        <?php if($product->availability>0 && $product->price!="0.00"){?>
                        <?php if($user_id['id']!=$product->user_id){?>
                        {!! Form::open(array('method' => 'POST', 'route' => array('add_cart', $product->id), 'class'=>'formCart','style'=>'float:left')) !!}
                        <button type="submit" class="btn btn-primary btn-sm" style="border:none;padding:10px;">
                            <i class="icon-shopping-cart"></i>{{ Lang::get('app.Add to cart') }}</button>
                        <?php } ?>
                        {!! Form::close() !!}
                        <?php } else { ?>
                        ...
                        <?php  }?>
                        <?php if($user_id['id']!=$product->user_id){?>
                        {!! Form::open(array('method' => 'POST', 'route' => array('add_wishlist', $product->id), 'class'=>'formCart','style'=>'float:left;')) !!}
    ...
    <section class="tabs-widget">
        ...
        <div id="productTabContent" class="tab-content container">
            ...
            <div class="tab-pane fade in active" id="adress_tabs">
                ...
                <?php } else { ?>
                <h2>{{ Lang::get('app.No Adress yet') }}!!!</h2>
                <?php } ?>
            </div>
    ...
    <div class="container">
        ...
        <div class="row">
            <?php if(sizeof($relatedtoUserShop)>0){?>

            <hr/>
            <h2>{{ Lang::get('app.Other products that this user have') }}</h2>
            @include('new_template.client.pages.productpartials.relateduserproducts')
            <?php } ?>
        </div>
        <div class="row">
            ...
            @include('new_template.client.pages.productpartials.relateduserother')
            <?php } ?>
        </div>
    </div>
