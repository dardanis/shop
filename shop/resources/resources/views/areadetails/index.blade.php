        ...
        <div class="col-lg-12">
            <section class="panel">
                <header class="panel-heading">
                    Areas {!! link_to_route('areas_create', 'Add Area', array(), array('class' => 'btn btn-primary btn-xs')) !!}
                </header>
                        ...
                        <table  class="display table table-bordered table-striped" id="example">
                            ...
                            @foreach($type as $t)
                                <tr data-link="row" class="rowlink">
                                    ...
                                    <td>{{$t->area_name}}</td>
                                    <td>{{$t->area_alias}}</td>
                                    <td>{{$t->area_description}}</td>

                                    <td><span class="label label-info">{{ Carbon::createFromTimestamp(strtotime($t->created_at))->diffForHumans() }}</span></td>

                                    <td>{!! link_to_route('edit_area', trans('shop.edit'), array($t->area_alias,$t->area_id), array('class' => 'btn btn-warning btn-xs')) !!}</td>

                                    <td>
                                        {!! Form::open(array('method' => 'DELETE', 'route' => array('delete_area', $t->area_id))) !!}
